...
if Selection==1
    ...
    prompt1 = {'Subject Name/Number:','Session:','Day:','Group:','Number of Blocks:',...
               'Target Display Time (ms):','Orientation','EEG?'};
    defaultans1 = {'','','','','','10','','Yes'};
else
    ResponseDevice = 'Keyboard';
    prompt1 = {'Subject Name/Number:','Session:','Day:','Group:','Number of Blocks:',...
               'Target Display Time (ms):','Orientation','EEG?','T/row keys:','L/column keys:'};
    defaultans1 = {'','','','','','10','','Yes','2','3'};
end
...
while ~begin
    ...
    if ~strcmpi(char(inputs1(7)),'-') && ~strcmpi(char(inputs1(7)),'|')
        h = warndlg('Orientation must be either "-" or "|"!');
        uiwait(h);
        begin = 0;
    end
    if ~strcmpi(char(inputs1(8)),'yes') && ~strcmpi(char(inputs1(8)),'no')
        h = warndlg('EEG must be either "Yes" or "No"!');
        uiwait(h);
        begin = 0;
    end
    SubjectName = char(inputs1(1));
    filename = ['EEG_TDT_Interferance_Experiment_Subject_',num2str(SubjectName),'_Results.mat'];
    if ~isempty(dir(filename))
        load(filename);
        if length(Parameters) == str2double(char(inputs1(2))) %#ok<NODEF>
            choice = questdlg(['EEG TDT Interferance Experiment results for subject "',SubjectName,'" Session ',char(inputs1(2)),' already exist and will be overwritten, continue?'],'!!WARNING!!','Yes','No','No');
            if strcmp(choice,'No')
                begin = 0;
            end
        end
...
if strcmp(Parameters(Session).ResponseDevice,'Keyboard')
    Parameters(Session).T_Hor = char(inputs1(9)); % Keys for identifying 'T' for fixation and horizontal for alignment.
    Parameters(Session).L_Ver = char(inputs1(10)); % Keys for identifying 'L' for fixation and vertical for alignment.
end
Parameters(Session).Orientation = char(inputs1(7));
Parameters(Session).EEG = char(inputs1(8));
if strcmpi(Parameters(Session).EEG,'Yes')
    EEG = 1;
else
    EEG = 0;
end
...
while b < Parameters(Session).nBlocks+1
    ...
    for t = 1:length(Times(Session).SOAs)
        ...
        flipTime = Screen('Flip', window1);
        if EEG
            io64(ioObj,LPT_address,100+b);
            pause(0.03);
            io64(ioObj,LPT_address,0);
            pause(0.03);
            io64(ioObj,LPT_address,200+t);
            pause(0.03);
            io64(ioObj,LPT_address,0);
        end
        ...
        if EEG
            io64(ioObj,LPT_address,find(Times(Session).SOAs==Output(Session).SOAs(b,t),1));
        end
    ...
    if Parameters(Session).Day==1 && b<4 && sum(Output(Session).SortedSuccesses(2,1:6,b))<5
        ...
        if b < Parameters(Session).nBlocks
            if strcmp(Parameters(Session).ResponseDevice,'Mouse')
...
if ~rem(Parameters(Session).nBlocks,3)
    ...
    if strcmp(choice,'Yes')
        ...
        for i = 1:(Parameters(Session).nBlocks/3)
            ...
            saveas(gcf,['Weibull_Graph_EEG_Interferance_Subject_',SubjectName,'_Session_Number_',num2str(Session),'_Third_Number_',num2str(i),'.fig']);
            saveas(gcf,['Z:\Yehuda\Backups\','Weibull_Graph_EEG_Interferance_Subject_',SubjectName,'_Session_Number_',num2str(Session),'_Third_Number_',num2str(i),datestr(now,'dd.mm.yy-HHMMSS'),'.fig']);
        end
        if (i>1)
            i = i+1;
            figure(1);
            ...
            WeibullAnalysis(Session).p_es(i) = temp(3);
            WeibullAnalysis(Session).t80(i) = fzero( 'WeibMinus', 0, [], temp(1), temp(2), temp(3), .8 );
            saveas(gcf,['Weibull_Graph_EEG_Interferance_Subject_',SubjectName,'_Session_Number_',num2str(Session),'_Total_Session.fig']);
            saveas(gcf,['Z:\Yehuda\Backups\','Weibull_Graph_EEG_Interferance_Subject_',SubjectName,'_Session_Number_',num2str(Session),'_Total_Session_',datestr(now,'dd.mm.yy-HHMMSS'),'.fig']);
        end
        close all;
