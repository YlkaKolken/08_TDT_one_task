...
% In each block the subject performs each  a given number of times
% (nTrials) in a random order. There are 14  times as a default.
...
choice.nTrials = 1; % Number of trials for each .
...
while 1
    ...
    if choice.TargetTime>40
        h = warndlg('Target time is more than the shortest  time!');
        uiwait(h);
        continue
    end
...
Times(Session).s = repmat([347 307 267 227 207 187 167 147 127 107 87 67 47],1,choice.nTrials); %  (Stimulus-to-mask Onset Asynchrony) times which will be used in each block.
...
if isempty(dir('D:\Ylka\TDT_censor-main\Data\'))
    question2 = questdlg(['Local backup folder "D:\Ylka\TDT_censor-main\Data\" not found and no local backups'...
...
if isempty(dir('D:\Ylka\TDT_censor-main\Data\'))
    question2 = questdlg(['Drive backup folder "D:\Ylka\TDT_censor-main\Data\" not found and no drive backups'...
                          ' will be saved, continue?'],'!!WARNING!!','Yes','No','No');
    if strcmp(question2,'No')
        return
    end
        ...
        while count < FixCtrTimeFrames
            ...
            if keyIsDown
                if eye_tracking && save_eye_data && count == 0
                    strtosend = ['dataFile_insertstring "LOC'  num2str(i+1) '"'];
...
%            Screen('FillRect', window1, [screen_back screen_back screen_back], []);
     ...
     while b < choice.nBlocks+1
         ...
         Output(Session).Displays(:,:,b) = 84*ones(2,length(Times(Session).s));
         Output(Session).Displays(2,:,b) = 45;
         Output(Session).s(b,:) = zeros(1,length(Times(Session).s));
         % Prepare the Output matrices with default values which will change
         % according to the subject's responses:
         Output(Session).Responses(:,:,b) = char(84*ones(2,length(Times(Session).s)));
         Output(Session).Responses(2,:,b) = '-';
         Output(Session).ResponseTimes(:,:,b) = zeros(2,length(Times(Session).s));
         Output(Session).Success(:,:,b) = zeros(length(Times(Session).s)/choice.nTrials,2);

         % Display the cross fixation:
         Screen('DrawTexture', window1, CrossDisplay);
         flipTime = Screen('Flip', window1);

         % randomizing the fixation, alignments and 's for the current block:
         order = zeros(length(Times(Session).s),1);
         order(round(length(Times(Session).s)/2)+1:end) = 1;
         order = order(randperm(length(Times(Session).s)));
         Output(Session).Displays(1,~order,b) = 76;
         order = order(randperm(length(Times(Session).s)));
         Output(Session).Displays(2,~order,b) = 124;
         Output(Session).s(b,:) = Times(Session).s(randperm(length(Times(Session).s)));
         % An array of textures to display:
         imageDisplay = zeros(length(Times(Session).s),2);

         % Prepare the textures for the targets and masks for all the trials in
         % the current block:
         for t = 1:length(Times(Session).s)
    ...
    for t = 1:length(Times(Session).s)
        ...
        if eye_tracking
            vpx_SendCommandString(['dataFile_insertmarker S']);
        end
        ...
        flipTime = Screen('Flip', window1, flipTime + Output(Session).s(b,t)/1000 - Times(Session).Target - Times(Session).slack);
        ...
        if eye_tracking
            vpx_SendCommandString(['dataFile_insertmarker R']);
        end
    ...
    [~,temp] = sort(Output(Session).s(b,:),'descend');
    ...
    % Average successes by :
    for t = 1:length(Times(Session).s)/choice.nTrials   
        Output(Session).Success(t,:,b) = mean(temp(choice.nTrials*(t-1)+1:choice.nTrials*t,:),1);
    end
    ...
    if SaveInitialTraining
        try
            save(['D:\Ylka\TDT_censor-main\Data\',filename,'_PreWeibull',datestr(now,'dd.mm.yy-HHMMSS'),'.mat'], 'SubjectName', 'ThresholdTraining', 'InitialTraining');
        catch
        ...
        save(['D:\Ylka\TDT_censor-main\Data\', filename, '_', datestr(now, 'dd.mm.yy-HHMMSS'), '.mat'], 'SubjectName', 'InitialTraining', 'ThresholdTraining');
    else
        try
            save(['D:\Ylka\TDT_censor-main\Data\',filename,'_PreWeibull',datestr(now,'dd.mm.yy-HHMMSS'),'.mat'], 'SubjectName', 'ThresholdTraining');
        catch
            fprintf('''D:\Ylka\TDT_censor-main\Data\'' folder does not exist, and no local backup saved!\n');
        end
        try
            save(['D:\Ylka\TDT_censor-main\Data\',filename,'_PreWeibull',datestr(now,'dd.mm.yy-HHMMSS'),'.mat'], 'SubjectName', 'ThresholdTraining');
        catch
            fprintf('''D:\Ylka\TDT_censor-main\Data\'' folder does not exist, and no drive backup saved!\n');
        end
        save(['D:\Ylka\TDT_censor-main\Data\', filename, '_', datestr(now, 'dd.mm.yy-HHMMSS'), '.mat'], 'SubjectName', 'ThresholdTraining');
    end
...
if SaveInitialTraining
    ...
    save(['D:\Ylka\TDT_censor-main\Data\', filename, '_', datestr(now, 'dd.mm.yy-HHMMSS'), '.mat'], 'SubjectName', 'ThresholdTraining', 'InitialTraining');
else
    try
        save(['D:\Ylka\TDT_censor-main\Data\',filename,'_PreWeibull',datestr(now,'dd.mm.yy-HHMMSS'),'.mat'], 'SubjectName', 'ThresholdTraining');
    catch
...
if strcmp(choice,'Yes')
    ...
    % The  times vector:
    T = sort(unique(Times(Session).s,'stable'),'descend')';
    figure;
    % The average success ratio per :
    y = mean(Output(Session).Success(:,2,:),3);
    ...
    % warning message displays:
    try
        saveas(gcf,['D:\Ylka\TDT_censor-main\Data\','Weibull_Graph_Subject_',SubjectName,'_Session_Number_',num2str(Session),datestr(now,'dd.mm.yy-HHMMSS'),'.fig']);
    catch
        fprintf('''D:\Ylka\TDT_censor-main\Data\'' folder does not exist, and no local backup figure saved!\n');
    end
    try
        saveas(gcf,['D:\Ylka\TDT_censor-main\Data\','Weibull_Graph_Subject_',SubjectName,'_Session_Number_',num2str(Session),datestr(now,'dd.mm.yy-HHMMSS'),'.fig']);
    catch
        fprintf('''D:\Ylka\TDT_censor-main\Data\'' folder does not exist, and no drive backup figure saved!\n');
    end
    saveas(gcf,['D:\Ylka\TDT_censor-main\Data\', 'Weibull_Graph_Subject_',SubjectName,'_Session_Number_',num2str(Session),'.fig']);
    ...
    if SaveInitialTraining
        try
            save(['D:\Ylka\TDT_censor-main\Data\',filename,'_PostWeibull',datestr(now,'dd.mm.yy-HHMMSS'),'.mat'], 'SubjectName', 'ThresholdTraining', 'InitialTraining');
        catch
            fprintf('''D:\Ylka\TDT_censor-main\Data\'' folder does not exist, and no local backup saved!\n');
        end
        try
        ...
        try
            save(['D:\Ylka\TDT_censor-main\Data\',filename,'_PostWeibull',datestr(now,'dd.mm.yy-HHMMSS'),'.mat'], 'SubjectName', 'ThresholdTraining');
        catch
            fprintf('''D:\Ylka\TDT_censor-main\Data\'' folder does not exist, and no local backup saved!\n');
        end
        try
...
% this subfunction.
    ...
    uicontrol('Parent',d,...
              ...
              'String',{'TMS Room';'TDT Room';'Old HP';'ilab2'},...
              'Callback',@getScreen);
